...
app.post("/signup", upload.single("avatar"), async (req: Request, res) => {
  const { email, password, name, username, phone } = req.body;
  const avatar = req.file ? req.file.filename : null;

  try {
    const user = await prisma.user.create({
      data: { email, password, name, username, phone, avatar },
    });
...
app.put("/users/:id", authenticateToken, upload.single("avatar"), async (req, res) => {
  ...
  // ✅ فقط حدّث الباسورد لو تم إرساله فعلاً وقيمته مش "placeholder"
  if (password && password !== "placeholder") {
    dataToUpdate.password = password;
    // ⚠️ مستحب: هنا تعمل هاش للباسورد الجديد قبل الحفظ
...
app.delete("/posts/:id", authenticateToken, async (req, res) => {
  ...
  try {
    // امسح الأقسام المرتبطة بالبوست
    await prisma.section.deleteMany({ where: { postId } });

    // بعد كدا امسح البوست نفسه
